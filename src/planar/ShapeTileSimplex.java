...
public final class ShapeTileSimplex extends ShapeTile
   ...
   public PlaPoint corner(int p_no)
      ...
      if (precalc_corners_int == null)  precalc_corners_int = new PlaPoint[arr.length];
      ...
      precalc_corners_int[p_no] = arr[p_no].intersection(prev);
   ...
   public PlaPointFloat corner_approx(int p_no)
      ...
      if (precalc_corners_float == null) precalc_corners_float = new PlaPointFloat[arr.length];
      ...
      precalc_corners_float[p_no] = arr[p_no].intersection_approx(prev);
   ...
   public PlaPointFloat[] corner_approx_arr()
      ...
      // corner array is not yet allocated
         {
         precalc_corners_float = new PlaPointFloat[arr.length];
         }
      for (int i = 0; i < precalc_corners_float.length; ++i)
         ...
         // corner is not yet calculated
            ...
            precalc_corners_float[i] = arr[i].intersection_approx(prev);
            }
         }
   ...
   public PlaDimension dimension()
      ...
      if (arr.length == 3)
         ...
         PlaPoint intersection = arr[1].intersection(arr[2]);
         PlaSide side_of_line0 = arr[0].side_of(intersection);
   ...
   public ShapeTileSimplex translate_by(PlaVector p_vector)
      ...
      PlaLineInt[] new_arr = new PlaLineInt[arr.length];
      for (int i = 0; i < arr.length; ++i)
         {
   ...
   public ShapeTileOctagon bounding_octagon()
      {
      if (precalc_bounding_octagon == null)
         {
         ...
         precalc_bounding_octagon = new ShapeTileOctagon((int) Math.floor(lx), (int) Math.floor(ly), (int) Math.ceil(rx), (int) Math.ceil(uy), (int) Math.floor(ulx), (int) Math.ceil(lrx),
   ...
   public ShapeTileSimplex enlarge(double p_offset)
      ...
      ShapeTileOctagon bounding_oct = this.bounding_octagon();
   ...
   public ShapeTileSimplex intersection(ShapeTileSimplex p_other)
      ...
      PlaLineInt[] new_arr = new PlaLineInt[arr.length + p_other.arr.length];
   ...
   public ShapeTileSimplex remove_border_line(int p_no)
      ...
      PlaLineInt[] new_arr = new PlaLineInt[this.arr.length - 1];
   ...
   public ShapeTileSimplex[] cutout_from(ShapeTileSimplex p_outer_simplex)
      ...
      ShapeTileSimplex inner_simplex = this.intersection(p_outer_simplex);
      ...
      PlaDirectionInt first_direction = (PlaDirectionInt) first_division_line.direction();
      ...
      for (int inner_corner_no = 0; inner_corner_no < inner_corner_count; ++inner_corner_no)
         ...
         if (curr_division_lines.length == 2)
            ...
            PlaDirectionInt curr_dir = (PlaDirectionInt) curr_division_lines[0].direction();
            boolean merge_prev_division_line = false;
            boolean merge_first_division_line = false;
            if (prev_division_line != null)
               {
               PlaDirectionInt prev_dir = (PlaDirectionInt) prev_division_line.direction();
            ...
            if (check_cross_first_line)
               {
               PlaDirectionInt curr_dir2 = (PlaDirectionInt) curr_division_lines[1].direction();
         ...
         PlaDirectionInt last_curr_dir = (PlaDirectionInt) last_curr_division_line.direction();
         ...
         if (check_cross_first_line)
            {
            PlaDirectionInt next_dir = (PlaDirectionInt) next_division_line.direction();
         ...
         if (merge_first_division_line)
            ++piece_line_count;
         PlaLineInt[] piece_lines = new PlaLineInt[piece_line_count];
         PlaLineInt curr_line = inner_simplex.arr[inner_corner_no];
   ...
   ShapeTileSimplex remove_redundant_lines()
      {
      PlaLineInt[] line_arr = new PlaLineInt[arr.length];
      ...
      while (try_again)
         ...
         for (int ind = 0; ind < new_length; ++ind)
            ...
            PlaDirectionInt prev_dir = (PlaDirectionInt) prev_line.direction();
            PlaDirectionInt next_dir = (PlaDirectionInt) next_line.direction();
            ...
            if (det != 0) // prev_line and next_line are not parallel
               ...
               // direction of next_line is bigger than direction of prev_line
                  ...
                  if (intersection_sides[ind] == PlaSide.ON_THE_LEFT)
                     {
                     PlaDirectionInt curr_dir = (PlaDirectionInt) curr_line.direction();
   ...
   private PlaLineInt[] calc_division_lines(int p_inner_corner_no, ShapeTileSimplex p_outer_simplex)
      {
      PlaLineInt curr_inner_line = this.arr[p_inner_corner_no];
      ...
      PlaDirectionInt first_projection_dir = PlaDirection.NULL;
      PlaDirectionInt second_projection_dir = PlaDirection.NULL;
      PlaDirectionInt prev_inner_dir = (PlaDirectionInt) prev_inner_line.direction().opposite();
      PlaDirectionInt next_inner_dir = (PlaDirectionInt) curr_inner_line.direction();
      int outer_line_no = 0;
      ...
      for (int ind = 0; ind < p_outer_simplex.arr.length; ++ind)
         ...
         PlaDirectionInt curr_projection_dir = (PlaDirectionInt) inner_corner.perpendicular_direction(outer_line);
         ...
         if (projection_visible)
            ...
            PlaDirectionInt curr_second_projection_dir = curr_projection_dir;
            ...
            if (second_division_necessary)
               ...
               while (!second_projection_visible)
                  ...
                  curr_second_projection_dir = (PlaDirectionInt) inner_corner.perpendicular_direction(p_outer_simplex.arr[tmp_outer_line_no]);
