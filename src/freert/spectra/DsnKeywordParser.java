 ...
 * @author Alfons Wirtz
 */
public class DsnKeywordParser extends DsnKeywordScope
   {

   /** Creates a new instance of Parser */
   public DsnKeywordParser()
      {
      super("parser");
      }
   ...
   public boolean read_scope(DsnReadScopeParameters p_par)
      ...
      for (;;)
         ...
         if (prev_token == OPEN_BRACKET)
            {
            if (next_token == DsnKeyword.STRING_QUOTE)
               {
            ...
            else if (next_token == DsnKeyword.HOST_CAD)
               {
               p_par.host_cad = DsnReadUtils.read_string_scope(p_par.scanner);
               }
            else if (next_token == DsnKeyword.HOST_VERSION)
               {
               p_par.host_version = DsnReadUtils.read_string_scope(p_par.scanner);
               }
            else if (next_token == DsnKeyword.CONSTANT)
               {
            ...
            else if (next_token == DsnKeyword.WRITE_RESOLUTION)
               {
               p_par.write_resolution = read_write_solution(p_par);
               }
            else if (next_token == DsnKeyword.GENERATED_BY_FREEROUTE)
               {
   ...
   private static DsnWriteResolution read_write_solution(DsnReadScopeParameters p_par)
      {
      try
         {
         ...
         return new DsnWriteResolution(resolution_string, resolution_value);
         }
      catch (java.io.IOException e)
         {
   ...
   private static String[] read_constant(DsnReadScopeParameters p_par)
      {
      try
         {
         String[] result = new String[2];
         p_par.scanner.yybegin(DsnFileScanner.NAME);
         Object next_token = p_par.scanner.next_token();
         if (!(next_token instanceof String))
            {
    ...
    */
   public static void write_scope(gui.varie.IndentFileWriter p_file, DsnParserInfo p_parser_info, freert.spectra.DsnIdentifier p_identifier_type, boolean p_reduced)
   ...
   private static String read_quote_char(JflexScanner p_scanner)
      {
      try
         {
