...
public class DsnRectangle extends DsnShape
    ...
    */
   public DsnRectangle(DsnLayer p_layer, double[] p_coor)
      {
      super(p_layer);
      coor = p_coor;
      }
    ...
    */
   public DsnRectangle union(DsnRectangle p_other)
      {
   ...
   public ShapeTileBox transform_to_board_rel(DsnCoordinateTransform p_coordinate_transform)
      ...
      if (box_coor[1] <= box_coor[3])
         ...
         result = new ShapeTileBox(box_coor[0], box_coor[1], box_coor[2], box_coor[3]);
         }
      else
         {
         // box_coor describe upper left and lower right corner
         result = new ShapeTileBox(box_coor[0], box_coor[3], box_coor[2], box_coor[1]);
         }
      return result;
      }
   ...
   public ShapeTileBox transform_to_board(DsnCoordinateTransform p_coordinate_transform)
      ...
      PlaPointFloat lower_left = p_coordinate_transform.dsn_to_board(curr_point);
      curr_point[0] = Math.max(coor[0], coor[2]);
      curr_point[1] = Math.max(coor[1], coor[3]);
      PlaPointFloat upper_right = p_coordinate_transform.dsn_to_board(curr_point);
      return new ShapeTileBox(lower_left.round(), upper_right.round());
      }

   /**
    * Writes this rectangle as a scope to an output dsn-file.
    */
   public void write_scope(IndentFileWriter p_file, DsnIdentifier p_identifier) throws java.io.IOException
      {
      p_file.new_line();
      p_file.write("(rect ");
   ...
   public void write_scope_int(IndentFileWriter p_file, DsnIdentifier p_identifier) throws java.io.IOException
      {
      p_file.new_line();
      p_file.write("(rect ");
      p_identifier.write(this.layer.name, p_file);
