...
public class BrdValidate
   ...
   public static boolean check(String p_s, RoutingBoard p_board)
      ...
      for (int layer = 0; layer < layer_count; ++layer)
         ...
         while (i.hasNext())
            {
            BrdItem curr_ob = (BrdItem) i.next();
            if (!curr_ob.validate_ok())
               {
   ...
   static public boolean check(String p_s, RoutingBoard p_board, Polyline p_polyline, int p_layer, int p_half_width, NetNosList p_net_no_arr, int p_cl_type)
      {
      ArrayList<ShapeTile> offset_shapes = p_polyline.offset_shapes(p_half_width, 0, p_polyline.corner_count());
      ...
      for (ShapeTile a_shape : offset_shapes )
         ...
         Collection<BrdItem> obstacles = p_board.search_tree_manager.get_default_tree().find_overlap_items_with_clearance(
               a_shape, p_layer, p_net_no_arr, p_cl_type);
         
         Iterator<BrdItem> it = obstacles.iterator();
         
         while (it.hasNext())
            {
            BrdItem curr_obs = it.next();
    ...
    */
   static public void orthogonal(String p_s, RoutingBoard p_board)
      {
      Iterator<BrdItem> it = p_board.get_items().iterator();
      while (it.hasNext())
         {
    ...
    */
   static public void multiple_of_45_degree(String p_s, RoutingBoard p_board)
      {
      int count = 0;
      Iterator<BrdItem> it = p_board.get_items().iterator();
      while (it.hasNext())
   ...
   static public int stub_count(String p_s, RoutingBoard p_board, int p_net_no)
      {
      if (first_time)
         {
   ...
   static public boolean has_cycles(String p_s, RoutingBoard p_board)
      {
    ...
    */
   static public boolean trace_count_exceeded(String p_s, RoutingBoard p_board, int p_net_no, int p_max_count)
      {
      int found_traces = 0;
      Iterator<BrdItem> it = p_board.get_items().iterator();
      while (it.hasNext())
    ...
    */
   static public boolean unconnnected_routing_items(String p_s, RoutingBoard p_board)
      {
      Iterator<BrdItem> it = p_board.get_items().iterator();
      while (it.hasNext())
         {
         ...
         if (curr_item.is_route())
            {
            Collection<BrdItem> contact_list = curr_item.get_normal_contacts();
