 ...
 * @author Alfons Wirtz
 */
public class WindowNetClasses extends GuiSubWindowSavable
   {
   ...
   private JComboBox<String> cl_class_combo_box;
   private JComboBox<String> via_rule_combo_box;
   ...
   public WindowNetClasses(BoardFrame p_board_frame)
      ...
      resources = board_frame.newGuiResources("gui.resources.WindowNetClasses");
      ...
      layer_combo_box = new ComboBoxLayer(routing_board.layer_structure, p_board_frame.get_locale());
      add_combobox_items();
   ...
   public void refresh()
      ...
      via_rule_combo_box.removeAllItems();
      add_combobox_items();
   ...
   private void add_combobox_items()
      ...
      for (RuleViaInfoList curr_rule : routing_board.brd_rules.via_rules)
         {
         via_rule_combo_box.addItem(curr_rule.rule_name);
         }
      }
   ...
   private class RemoveNetClassListener implements java.awt.event.ActionListener
      ...
      public void actionPerformed(java.awt.event.ActionEvent p_evt)
         ...
         if (board_rules.net_classes.remove(net_rule))
            {
            adjust_table();
            String message = resources.getString("net_class") + " " + net_rule.get_name() + " " + resources.getString("removed");
            board_frame.screen_messages.set_status_message(message);
            }
         }
      }
   ...
   private class ContainedNetsListener implements java.awt.event.ActionListener
      ...
      public void actionPerformed(java.awt.event.ActionEvent p_evt)
         ...
         java.util.Collection<PrintableInfo> contained_nets = new java.util.LinkedList<PrintableInfo>();
         int max_net_no = board_rules.nets.max_net_no();
         for (int i = 1; i <= max_net_no; ++i)
            {
            rules.RuleNet curr_net = board_rules.nets.get(i);
   ...
   private class NetClassTableModel extends javax.swing.table.AbstractTableModel
      ...
      public void set_values()
         ...
         for (int i = 0; i < data.length; ++i)
            ...
            if (curr_net_class.get_via_rule() != null)
               {
               this.data[i][ColumnName.VIA_RULE.ordinal()] = curr_net_class.get_via_rule().rule_name;
               }
            ...
            GuiLayer combo_layer = layer_combo_box.get_selected_layer();
      ...
      void set_trace_width_field(int p_rule_no, GuiLayer p_layer)
         ...
         if (p_layer.index == ComboBoxLayer.ALL_LAYER_INDEX)
            ...
            if (curr_net_class.trace_width_is_inner_layer_dependent())
               ...
               while ( ! layer_structure.is_signal(first_inner_signal_layer_no))
                  {
                  ++first_inner_signal_layer_no;
                  }
               if (first_inner_signal_layer_no < layer_structure.size() - 1)
                  {
      ...
      public void setValueAt(Object p_value, int p_row, int p_col)
         ...
         if (p_col == ColumnName.NAME.ordinal())
            ...
            String new_name = (String) p_value;
            RuleViaInfoList new_via_rule = board_rules.get_via_rule(new_name);
            ...
            {
            if (!(p_value instanceof GuiLayer))
               {
               return;
               }
            set_trace_width_field(p_row, (GuiLayer) p_value);
            }
