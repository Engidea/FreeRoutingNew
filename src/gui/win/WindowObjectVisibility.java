...
public class WindowObjectVisibility extends WindowVisibility
   ...
   public static WindowObjectVisibility get_instance(BoardFrame p_board_frame)
      {
      java.util.ResourceBundle resources = java.util.ResourceBundle.getBundle("gui.resources.WindowObjectVisibility", p_board_frame.get_locale());
      String title = resources.getString("title");
      String header_message = resources.getString("header_message");
      String[] message_arr = new String[ColorIntensityName.values().length];
      for (int i = 0; i < message_arr.length; ++i)
         {
         message_arr[i] = resources.getString(ColorIntensityName.values()[i].toString());
         }
   ...
   protected void set_changed_value(int p_index, double p_value)
      ...
      get_board_handling().gdi_context.color_intensity_table.set_value(p_index, p_value);
      }
   }
