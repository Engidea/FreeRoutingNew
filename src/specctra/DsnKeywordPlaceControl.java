 ...
 * @author Alfons Wirtz
 */
public class DsnKeywordPlaceControl extends DsnKeywordScope
   {

   public DsnKeywordPlaceControl()
      {
      super("place_control");
      }
   ...
   public boolean read_scope(DsnReadScopeParameters p_par)
      ...
      for (;;)
         ...
         if (prev_token == OPEN_BRACKET)
            {
            if (next_token == DsnKeyword.FLIP_STYLE)
               {
    ...
    */
   static boolean read_flip_style_rotate_first(JflexScanner p_scanner)
      {
      try
         {
         boolean result = false;
         Object next_token = p_scanner.next_token();
         if (next_token == DsnKeyword.ROTATE_FIRST)
            {
            if (next_token == DsnKeyword.ROTATE_FIRST)
               {
               result = true;
